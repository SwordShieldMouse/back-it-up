...
def main():
    ...
    # (batch_size, N_param)
    pi_actions, pi_logprob = sample_action_and_logprob(all_candidates, config.N_param)
    q_val = env.reward_func(pi_actions)
        ...
        if not args.load_results:
            for t_idx, tau in enumerate(config.entropy_scale):
                ...
                if tau == 0:
                    ...
                    optimal_action = 0.5  # optimal peak
                    tiled_actions = torch.tensor(np.tile(optimal_action, [batch_size, ]),
                                                 requires_grad=True)  # (1022, )

                    max_a_logprob = compute_pi_logprob(all_candidates, tiled_actions)
                    ...
                    with torch.no_grad():
                        max_arg = torch.max(q_val / tau - pi_logprob, axis=0, keepdim=True)[0]
                        rho = torch.exp(q_val / tau - pi_logprob - max_arg)
                    # print(rho.shape, q_val.shape, pi_logprob.shape)
                    WIR = rho / rho.sum(-1, keepdim=True)

                    # (batch_size, 1022)
                    loss = - torch.sum(WIR * pi_logprob, axis=-1)
    ...
    if args.compute_kl_type == 'reverse':
        ...
        if not args.load_results:
            for t_idx, tau in enumerate(config.entropy_scale):
                ...
                if tau == 0:
                    loss = - torch.mean(q_val * pi_logprob, axis=-1)
                else:
                    # (1022, )
                    loss = torch.mean(pi_logprob + pi_logprob.detach() * pi_logprob - q_val * pi_logprob / tau, axis=-1)
...
def sample_action_and_logprob(mean_std_batch, num_actions):

    permuted_mean_std_batch = mean_std_batch.permute(1, 0)  # ( 2, batch_size)
    dist = Normal(permuted_mean_std_batch[0], F.softplus(permuted_mean_std_batch[1]))

    sampled_actions = dist.sample(torch.Size([num_actions]))
    sampled_logprob = dist.log_prob(sampled_actions)
    sampled_actions, sampled_logprob = torch.tanh(sampled_actions.permute(1, 0)), sampled_logprob.permute(1, 0)
    sampled_logprob -= torch.log(1-torch.pow(sampled_actions, 2))

    return sampled_actions, sampled_logprob
...
def compute_plot(kl_type, entropy_arr, x_arr, y_arr, kl_arr, grad_arr, save_dir):
    ...
    for t_idx, tau in enumerate(entropy_arr):
        ...
        # plt.savefig('{}/{}_kl_{}_tau={}_lined.png'.format(save_dir, kl_type, t_idx, tau))
        # plt.clf()
