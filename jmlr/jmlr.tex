...
% SS: Update the above claim after reviewing the literature mentioned by NeurIPS reviewers
...
\begin{align}
%\boltzmannQ(s, a) \propto \exp(\Qhat(s, a)\tau^{-1})
...
Define the \textbf{Reverse KL} for greedification at a given state $s$ and action-value $\Qhat$ ($\Qhat$ may be soft or not):%, with corresponding gradient, as
%
\begin{align*}
 \text{RKL}(\policyparams; s, \Qhat) &\defeq \KL\left( \pi_\policyparams(\cdot \mid s_t) \parallel \boltzmannQ(s_t, \cdot) \right)% \propto -\entropy(\pi_\policyparams(\cdot \mid s)) - \tau^{-1}\int_\actionspace  \pi_\policyparams(a \mid s) {Q(s, a)}\, da 
...
If we view the action-value Q as fixed, the gradient of \Cref{eq:hard-reverse-KL} is exactly the inner term of the policy gradient in \Cref{eq:policy-gradient-thm}.
