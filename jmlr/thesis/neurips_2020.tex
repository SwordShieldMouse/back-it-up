...
%Policy gradient methods typically estimate both explicit policy and value functions. The long-extant view of policy gradient methods as approximate policy iteration---alternating between policy evaluation and policy improvement---is a helpful frame to elucidate algorithmic choices. In particular, many modern policy gradient methods can be classified by their choice of how to perform approximate greedification of a value function estimate for policy improvement. In this work, we identify one such choice: the use of reverse or forward KL as an objective. We construct a counterexample where forward KL can fail to induce policy improvement in the tabular setting, which contrasts the known policy improvement result for the reverse KL. We also show, however, that forward KL can induce policy improvement under additional assumptions. Finally, we empirically analyze the optimization behaviour and practical performance of our different algorithms. We observe few consistent difference between the reverse and forward KLs on discrete action spaces, but more substantial stability and convergence differences emerge in continuous action spaces. 
\end{abstract}

% TODOS:
% 5b Second editting of expriments (Martha)
% 8. Revisit need for |A| less than infinity, PI improvement for forward KL with tau = 0, and only expected improvement rather than per state improvement for FKL (Alan, Martha)
% 9. Revisit scaling of RKL and FKL.
...
% MARTHAC: In the end, we do not explore conservative greedification, and arguably this is orthogonal anyway, since the same greedification operators would be used. 
%The most commonly used greedification operators are to set the policy to a Boltzmann distribution on the action-values \citep{wagner2011reinterpretation} or to minimize a KL divergence to that Boltzmann distribution \citep{haarnoja2018soft}, and in some cases to use a conservative greedification where the policy is not changed too much between steps, as in conservative policy iteration (CPI) \citep{kakade2002approximately,vieillard2019deep} and trust-region methods \citep{schulman2015trust,schulman2017proximal}.

% MARTHAC: This is too long. Where can we include some of this information?
%There is increasing work in the theoretical analysis of PG methods. CPI is an early example of theoretical insights into obtaining guaranteed policy improvement with PG \citep{kakade2002approximately}. More recently, \citet{agarwal2019optimality} derive finite-sample and approximation bounds for a variety of policy gradient methods; \citet{neu2017unified,liu2019neural} reformulate TRPO as mirror descent and prove convergence; \citet{ahmed2018understanding} show that entropy regularization may lead to smoother optimization landscapes; and \citet{bhandari2019global}\footnote{We note that the results in \citet{bhandari2019global} bear a striking resemblance to earlier results in \citet{scherrer2014local}. In particular, both works rely on convex policy classes and a closure of the policy class under greedification.} show global optimality of local minima under certain restrictions on the MDP.
%% MARTHA todo: There must be some works that have lookd in this. What are they?
%Empirically, however, there has been less insight into the impact of different choices with PG, namely into the impact of different greedification approaches and policy evaluation approaches. In fact, many implementations use an incorrect, simplified gradient \citep{thomas2014bias,imani2018off,nota2019policy}, and it has been shown that improvements for some methods may be due to code-level optimizations rather than algorithmic advances \citep{ilyas2018deep,engstrom2019implementation}.
%%The policy oscillation phenomon \citep{bertsekas} was empirically investigated under API, showing that parameterized stochastic policies set to Boltzmann distributions mitigated this oscillation compare to exact greedification \citep{wagner2011reinterpretation}. 

% MARTHA: Should we change the above that API has mostly focused on convergence of the whol system, with little insight into the greedification operator?
...
% MARTHAC: Too much here. They will see this later, especially in the appendix
%The update to the policy for many PG methods can be seen as optimizing a reverse KL, though there has been some work using a forward KL \citep{norouzi2016reward,nachum2016improving,agarwal2019learning,vieillard2019deep}, including implicitly some of the work using classification for RL \citep{lagoudakis2003reinforcement,lazaric2010analysis,farahmand2015classificationbased}. 
...
% AC: edits for making it punchier and avoiding duplication of what we'll say
We provide some clarity on this question in this work, with the following contributions. First, we highlight four choices for greedication: forward or reverse KL to a Boltzmann distribution on the action-values, with or without entropy regularization. %We categorize many PG methods as API with approximate greedification corresponding to one of these variants. This motivates understanding this more high-level choice, to improve PG algorithms and to compare existing algorithms. 
...
%\begin{itemize}[itemsep=1pt,topsep=0pt,leftmargin=8pt]
%    % MARTHAC: This is so cute, but maybe slightly inaccurate? Should it be: We unify various policy gradient updates as policy improvement steps with KL objectives  
%    % \item We unify various policy gradient updates under the umbrella of KL optimization. 
%    \item We rederive the vanilla policy gradient update as optimization of a KL objective and describe four algorithmic variants that can result from KL optimization.
%    \item We partially extend the policy improvement result in soft actor-critic to function-approximation settings. 
%    \item We provide a counterexample where optimizing for the forward KL can fail to induce policy improvement. Nevertheless, we show that under some additional conditions, optimizing for the forward KL can induce policy improvement.
%    % \item We discuss when the forward KL divergence can act as a suitable surrogate for the reverse KL divergence in the function-approximation regime.
%    \item We empirically investigate the behaviour of the reverse and forward KL in both understandable environments and popular RL control benchmarks.
%\end{itemize}


% OLD STUFF: LEAVING IT HERE TO REFERENCE LATER
% \textit{Policy gradient} is a popular approach for continuous control in \textit{reinforcement learning} (RL) \citep{sutton2018reinforcement}. The controller, otherwise known as the \textit{policy}, is parameterized by parameters that are modified iteratively to maximize a given objective function. In its most basic form, the direction of modification is given by the \textit{policy gradient theorem} \citep{sutton2000policy} that maximizes either the discounted return or the average reward. Policy gradient may also be interpreted as an approximate version \citep{bertsekas2011approximate,scherrer2014approximate} of \textit{policy iteration} \citep{sutton2018reinforcement}, which attempts to find an optimal policy by interleaving policy evaluation and policy improvement. To perform policy improvement, one must choose a ``policy improvement operator''; given a value function estimate, the output of such an operator is a policy that is (approximately) greedy with respect to that value function estimate. Some choices of an operator include $\epsilon$-greedy \citep{sutton2018reinforcement}, Boltzmann policies \citep{wagner2011reinterpretation}, and conservative policy iteration (CPI) approaches \citep{kakade2002approximately,vieillard2019deep}; we direct the reader to the surveys \cite{bertsekas2011approximate} or \citet{scherrer2014approximate} for a broader discussion on this point. The choice of a policy improvement operator has consequences for convergence and stability \citep{perkins2002existence,perkins2003convergent,wagner2013optimistic}. 

% Although not a recent invention \citep{sutton2000policy}, policy gradient methods have recently seen a surge of renewed interest given their ease of application in high-dimensional continuous action spaces when combined with neural networks \citep{schulman2015high,wang2016sample}.

% Several different policy gradient methods have been proposed in recent years: \textit{(deep) deterministic policy gradient} uses a deterministic policy gradient theorem to approximate a deterministic optimal policy \citep{silver2014deterministic,lillicrap2015continuous}; \textit{trust region policy optimization (TRPO)} \citep{schulman2015trust} and its variant \textit{proximal policy optimization (PPO)} \citep{schulman2017proximal} optimize a surrogate objective to ensure that policy updates do not stray too far from the current data collected; other methods attempt to perform approximate Q-learning \citep{watkins1992q} in continuous action space \citep{ haarnoja2017reinforcement,lim2018actor}; and lastly, some methods optimize a maximum-entropy objective that augment the standard expected return objective with an entropy term \citep{haarnoja2017reinforcement, haarnoja2018soft}. Methods with this maximum-entropy objective have also been shown to have preliminary connection to value-based methods \citep{nachum2017bridging, schulman2017equivalence, nachum2019algaedice}. Indeed, there has been much work in general on connecting policy gradient methods to value-based methods \citep{kakade2002natural,wagner2013optimistic,o2016combining,schulman2017equivalence,lim2018actor}.

% Despite this flurry of work, there is still generally a lack of understanding on what design choices, whether in the objective function or optimization algorithm, are relevant for good performance. Indeed, the improvements claimed by some methods may be due to code-level optimizations rather than algorithmic advances \citep{ilyas2018deep,engstrom2019implementation}. Recent work from the community is encouraging: \citet{agarwal2019optimality} derive finite-sample and approximation bounds for a variety of policy gradient methods; \citet{neu2017unified,liu2019neural} reformulate TRPO as mirror descent and prove convergence; \citet{ahmed2018understanding} show that entropy regularization may lead to smoother optimization landscapes. Our work also follows the long line  Nevertheless, more work should be done into understanding the effect of each design choice.

% In this paper, we focus on understanding optimization of KL objectives as an approximate policy improvement operator \citep{perkins2003convergent}, as done in soft actor-critic \citep{haarnoja2018soft} for example. 
% %Soft actor-critic has been widely used due to its stability, performance, relative simplicity and theoretical grounding \citep{haarnoja2018soft}. To maximize the entropy-augmented objective, soft actor-critic updates its policy by minimizing the reverse KL divergence to the Boltzmann distribution of its soft action-values. 
% Although minimizing the reverse KL is essential for the policy improvement result in the tabular setting \citep[p.~4]{haarnoja2018soft}, it is unclear whether the forward KL or reverse KL is more desirable in the regime of function-approximation. The choice of KL divergence is significant as it may greatly impact the optimization landscape of the objective and thus the stability of training and the quality of the resulting policy. In the setting of contextual bandits, \citet{chen2019surrogate} showed that optimizing a composite surrogate objective, involving the forward KL divergence, can achieve better performance than directly optimizing the original objective. Indeed, the forward KL divergence is used in supervised learning, in the form of the cross-entropy loss, rather than the reverse KL divergence. 
% % AC: Citation for cross-entropy loss needed?
% %More generally, an arbitrary choice of loss function can lead to incredibly difficult optimization problems \citep{auer1996exponentially}. 

% % In maximum-entropy policy gradient methods so far, the reverse KL divergence is used due to the more tractable sampling from the parameterized policy. 

% \subsection{Contributions}
% % To understand this divide, in this work we propose a way of approximating the forward KL divergence update without explicitly sampling from the intractable distribution and without importance sampling. This allows us to compare the relative advantages of using the forward or the reverse KL divergence objective in reinforcement learning. 
% % AC: I think this sort of sounds like the alg is our main contribution, but it's more of an experimental device? Not sure if we should mention it here
% % SS: I think it should be one of our contributions though, as it hasn't been done before. But yeah we should not make it our main contribution.
% % AC: As it is phrased, it makes it sound like this is a new state-of-the-art alg, which is not what we want. I'm not entirely sure that it's *that* novel an idea. Numerical integration is a common technique and was used in the all-actions workshop paper.

% %We analyze the comparative advantages of each KL divergence in soft actor-critic, both empirically and theoretically. Specifically, 
% We have the following contributions. 
% \begin{itemize}
%     \item We unify various policy gradient objectives under the umbrella of KL optimization.
%     \item We provide a counterexample where optimizing for the forward KL in a tabular setting can fail to induce policy improvement.
%     \item We partially extend the policy improvement result in soft actor-critic to function-approximation settings. 
%     \item We discuss when the forward KL divergence can act as a suitable surrogate for the reverse KL divergence in the function-approximation regime.
%     \item We include tabular and function-approximation results that highlight the success and failure cases of reverse and forward KL. %Instead of sampling from the typically intractable distribution involved in the forward KL in continuous action spaces, we utilize numerical integration methods instead, similar to recent work in all-actions policy gradient \citep{petit2019all}. 
% \end{itemize}
...
Because we do not have access to $Q^{\pi_\policyparams}$, approximations are used instead. %For example, in REINFORCE \citep{williams1992simple}, a sampled return from $(s,a)$ is used as an unbiased estimate of $Q^{\pi_\policyparams}(s,a)$. 
...
% AC: don't need to say state s and action-value Q since we already do so above
Similarly, we can define the (scaled) \textbf{Forward KL} for greedification %at a given state $s$ and action-value $\Qhat$: %this time omitting the entropy term which does not involve $\policyparams$
%
...
\begin{ack}
% The authors gratefully acknowledge support from the Reinforcement Learning and Artificial Intelligence Lab (RLAI) and the Alberta Machine Intelligence Institute (amii). The authors would also like to thank Dale Schuurmans for preliminary discussion on the choice of reverse or forward KL. 
\end{ack}
